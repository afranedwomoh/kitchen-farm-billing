  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 sm:justify-end">
        ...
        <Button variant="outline" className="h-11" onClick={() => ref.current && exportNodeAsPdf(ref.current, `${fileBaseName(data)}.pdf`)} disabled={!data}><FileDown className="h-4 w-4 mr-1.5" /> PDF</Button>
        {!converted && <Button className="h-11" onClick={convert} disabled={busy || !data}><ArrowRightCircle className="h-4 w-4 mr-1.5" /> Convert to invoice</Button>}
      </div>
